...
def metamodels_create_and_train(tasks, list_optimizers, limit_tasks_by_model = 16):
    ...
    for batch_task in batch_tasks:
        ...
        for opt in list_optimizers:
            ...
            nbatches_train, mod = divmod(len(X_train_scaled), params.BATCH_SIZE_PRINCIPAL)
            nbatches_valid, mod = divmod(len(X_val_scaled), BATCH_SIZE_PRINCIPAL)
            
            print('####################### FITTING METAMODEL FOR %s#############################'%opt.type)
            history = metamodel.fit_generator(generator = train_generator, steps_per_epoch = nbatches_train // hvd.size(), \
                                              validation_data = val_generator, validation_steps = 3 * nbatches_valid // hvd.size(),\
